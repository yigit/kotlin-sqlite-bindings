...
import com.birbit.ksqlite3.AuthResult
import com.birbit.ksqlite3.AuthorizationParams
import com.birbit.ksqlite3.Authorizer
import com.birbit.ksqlite3.SqliteException
...
internal class JvmAuthorizerCallback private constructor(
    ...
    companion object : JniCache {
        ...
        override fun init(scope: MemScope, env: CPointer<JNIEnvVar>) {
            scope.run {
                val classRef = obtainGlobalReference(env, "com/birbit/ksqlite3/Authorizer")
                val jvmAuthCallback = JvmAuthorizerCallback(
                    ...
                    invokeMethodId = getMethodId(
                        ...
                        classRef.jobject,
                        "invoke",
                        "(Lcom/birbit/ksqlite3/AuthorizationParams;)I"
                    ),
...
internal class JvmAuthorizerParams private constructor(
    ...
    companion object : JniCache {
        ...
        override fun init(scope: MemScope, env: CPointer<JNIEnvVar>) {
            scope.run {
                val classRef = obtainGlobalReference(env, "com/birbit/ksqlite3/AuthorizationParams")
    ...
    companion object : JniCache {
        ...
        override fun init(scope: MemScope, env: CPointer<JNIEnvVar>) {
            val jvmSqliteException = scope.run {
                val classRef = obtainGlobalReference(env, "com/birbit/ksqlite3/SqliteException")
                JvmSqliteException(
                    classRef = classRef,
